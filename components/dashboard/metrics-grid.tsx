...
export function DashboardMetricsGrid() {
    ...
    const metricsData: MetricCardProps[] = [
            ...
            footer: (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-primary font-bold text-sm">
                        +{portfolio.performancePercentage}% vs last month
                    </span>
                </div>
            ),
        },
        {
            title: "Total Income",
            value: `$${portfolio.totalIncome.toLocaleString()}`,
            footer: (
                <>
                    <div className="w-full h-1.5 bg-surface-container-lowest rounded-full overflow-hidden">
                        <div
                            className="h-full bg-primary shadow-[0_0_8px_rgba(172,199,255,0.4)]"
                            style={{
                                width: `${Math.min(((portfolio.totalIncome || 0) / 200000) * 100, 100)}%`,
                            }}
                        ></div>
                    </div>
                    <p className="text-on-surface-variant text-xs mt-3">
                        ...
                        {Math.min(
                            Math.round(
                                ((portfolio.totalIncome || 0) / 200000) * 100,
                            ),
                            100,
                        )}
                        % of annual projection
                    </p>
                </>
            ),
        },
        {
            ...
            value: `$${portfolio.totalExpenses.toLocaleString()}`,
            footer: (
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-tertiary font-medium text-sm">
                        {portfolio.totalExpenses > 0
                            ? `${Math.round((portfolio.totalExpenses / portfolio.totalIncome) * 100)}% of income`
                            : "No expenses"}
                    </span>
                </div>
            ),
